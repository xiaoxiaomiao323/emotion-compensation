...
def train_boundary(latent_codes,
  ...
  positive_num = np.sum(scores[:, 0] > 0)
  negative_num = len(scores) - positive_num
  train_pos_num = int(positive_num * split_ratio)
  val_pos_num = positive_num - train_pos_num
  train_neg_num = int(negative_num * split_ratio)
  val_neg_num = negative_num - train_neg_num

  positive_idx = np.where(scores[:, 0] > 0)[0]
  # print(scores, positive_idx)
  np.random.shuffle(positive_idx)
  positive_train = latent_codes[positive_idx[:train_pos_num]]
  positive_val = latent_codes[positive_idx[train_pos_num:]]

  negative_idx = np.where(scores[:, 0] == 0)[0]
  np.random.shuffle(negative_idx)
  negative_train = latent_codes[negative_idx[:train_neg_num]]
  negative_val = latent_codes[negative_idx[train_neg_num:]]
  ...
  # print(f'  Chosen number of samples: {chosen_num}')
  ...
  train_label = np.concatenate([np.ones(train_pos_num, dtype=int),
                                np.zeros(train_neg_num, dtype=int)], axis=0)
  logger.info(f'  Training: {train_pos_num} positive, {train_neg_num} negative.')
  # Validation set.
  val_data = np.concatenate([positive_val, negative_val], axis=0)
  val_label = np.concatenate([np.ones(val_pos_num, dtype=int),
                              np.zeros(val_neg_num, dtype=int)], axis=0)
  logger.info(f'  Validation: {val_pos_num} positive, {val_neg_num} negative.')
  # Remaining set.
